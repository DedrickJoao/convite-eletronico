import { WeddingEvent, PlaylistItem, GiftOption, DressCodePalette } from '../types';

export const WEDDING_DETAILS = {
  bride: "Benedita",
  groom: "Dionísio",
  monogram: "B & D",
  hashtag: "#BeneditaEDionisio2026",
  tagline: "O Casamento do Ano • Edição Exclusiva 2026",
  subTagline: "Onde o Amor se Transforma em Realeza",
  impactPhrase: "Dois corações que se escolheram para viver a eternidade. Convidamos você a testemunhar o casamento mais grandioso, sofisticado e inesquecível de 2026.",
  dateText: "Sábado, 12 de Dezembro de 2026",
  targetDate: "2026-12-12T16:00:00",
  ceremonyTime: "16:00",
  receptionTime: "18:30",
  venueName: "Palácio Imperial das Esmeraldas & Cristais",
  venueAddress: "Avenida Real dos Nobres, 1200 - Jardins Imperial",
  cityCountry: "Lisboa / Luanda / Sintra Luxury Estate",
  googleMapsUrl: "https://maps.google.com/?q=Sintra+Palace+Estate",
  appleMapsUrl: "https://maps.apple.com/?q=Sintra+Palace+Estate",
  wazeUrl: "https://waze.com/ul?q=Sintra+Palace+Estate",
  uberRideUrl: "https://m.uber.com/ul/?action=setPickup&pickup=my_location",
  spotifyPlaylistUrl: "https://open.spotify.com/playlist/37i9dQZF1DX4sWSpwq3LiO",
  organizerContact: "+351 912 345 678",
  organizerEmail: "vip@beneditaedionisio2026.com",
  rsvpDeadline: "15 de Outubro de 2026",
  nfcCardIdPrefix: "VIP-BD-2026-",
  bankingDetails: {
    iban: "PT50 0033 0000 1234 5678 9012 3",
    swift: "BCOMPTPL",
    holder: "Benedita & Dionísio Wedding Fund",
    pixKey: "casamento@beneditaedionisio.com",
    multicaixaExpress: "+244 923 456 789"
  }
};

export const TIMELINE_EVENTS: WeddingEvent[] = [
  {
    title: "Chegada dos Convidados de Honra",
    time: "15:30",
    location: "Jardim das Fontes Reais",
    address: "Pátio Principal com Valet Parking e Tapete Dourado",
    description: "Recepção com harpa ao vivo, flute de boas-vindas com Champagne Dom Pérignon e check-in VIP com leitura NFC.",
    iconName: "Sparkles"
  },
  {
    title: "Cerimónia Solene & Votos Eternos",
    time: "16:00",
    location: "Catedral de Cristal dos Jardins",
    address: "Altar Principal adornado em Flores Brancas e Folhas de Ouro",
    description: "A entrada triunfal dos noivos, troca de alianças personalizadas e bênção matrimonial sob uma chuva de pétalas de rosas.",
    iconName: "HeartHandshake"
  },
  {
    title: "Cocktail D'Ouro & Brinde ao Pôr do Sol",
    time: "17:30",
    location: "Terraço Panorâmico Royal",
    address: "Miradouro dos Cristais",
    description: "Performance exclusiva de saxofone ao vivo, mesa de ostras e caviar, mixologia molecular e sessão de fotos com os noivos.",
    iconName: "GlassWater"
  },
  {
    title: "O Grande Banquete Gastronómico",
    time: "19:30",
    location: "Salão Nobre das Esmeraldas",
    address: "Mesas Imperiais iluminadas por 1.000 velas aromáticas",
    description: "Menu de 5 tempos assinado por Chef Estrela Michelin com harmonização de vinhos de reserva exclusiva de 2026.",
    iconName: "UtensilsCrossed"
  },
  {
    title: "Corte do Bolo & Espetáculo Pirotécnico",
    time: "22:30",
    location: "Lado do Lago Espelhado",
    address: "Torre Imperial de Cristais",
    description: "Bolo de 7 andares folheado a ouro comestível 24k com espetáculo de fogos de artifício e show de luzes laser sincronizado.",
    iconName: "Cake"
  },
  {
    title: "Gala After-Party & Pista de Dança",
    time: "23:00 - Até ao Amanhecer",
    location: "Grand Ballroom & Club Lounge VIP",
    address: "Pista de Dança com Efeito Infinito e LED Glass",
    description: "DJs Internacionais, banda de soul/afrobeat ao vivo, bar de coquetéis premium all night e surpresas inesquecíveis.",
    iconName: "Music"
  }
];

export const PLAYLIST_ITEMS: PlaylistItem[] = [
  {
    id: "1",
    title: "A Thousand Years (Royal Orchestra Version)",
    artist: "The Royal Symphony Strings",
    duration: "4:45",
    vibe: "Entrada Nupcial & Emoção Pura",
    coverArt: "https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=400&q=80"
  },
  {
    id: "2",
    title: "Can't Take My Eyes Off You (Acoustic Jazz)",
    artist: "Benedita & Dionísio Golden Trio",
    duration: "3:58",
    vibe: "Primeira Dança dos Recém-Casados",
    coverArt: "https://images.unsplash.com/photo-1465847899084-d164df4dedc6?w=400&q=80"
  },
  {
    id: "3",
    title: "Perfect Symphony (Italian & Portuguese Duet)",
    artist: "Royal Chamber Players",
    duration: "4:22",
    vibe: "Brinde de Cristal & Champagne",
    coverArt: "https://images.unsplash.com/photo-1519741497674-611481863552?w=400&q=80"
  },
  {
    id: "4",
    title: "Love Someone (Lukas Graham Cello Cover)",
    artist: "Starlight Strings Ensemble",
    duration: "3:30",
    vibe: "Momento dos Votos e Alianças",
    coverArt: "https://images.unsplash.com/photo-1511285560929-80b456fea0bc?w=400&q=80"
  },
  {
    id: "5",
    title: "Stand By Me (Soul & Saxophone Edition)",
    artist: "Midnight Velvet Band",
    duration: "4:10",
    vibe: "Abertura do Coquetel Imperial",
    coverArt: "https://images.unsplash.com/photo-1520854221256-17451cc331bf?w=400&q=80"
  }
];

export const DRESS_CODE_PALETTE: DressCodePalette[] = [
  {
    name: "Ouro Imperial",
    colorHex: "#D4AF37",
    description: "Toques luxuosos em joias, vestidos e detalhes de gala."
  },
  {
    name: "Branco Seda & Pérola",
    colorHex: "#FAF9F6",
    description: "Exclusivo para a noiva e camisaria nobre dos cavalheiros."
  },
  {
    name: "Preto Black Tie",
    colorHex: "#111216",
    description: "Smoking clássico com laço de cetim para os senhores."
  },
  {
    name: "Esmeralda Nobre",
    colorHex: "#0D382B",
    description: "Opção majestosa para vestidos longos de alta costura."
  },
  {
    name: "Champagne Suave",
    colorHex: "#EED9B3",
    description: "Harmonia elegante para convidadas e damas de honra."
  }
];

export const GIFT_OPTIONS: GiftOption[] = [
  {
    id: "maldives",
    title: "Noites de Sonho em Bangalô sobre as Águas (Maldivas)",
    category: "Lua de Mel Exclusiva",
    amount: "€ 500 / Aporte Livre",
    description: "Proporcione aos noivos dias paradisíacos no resort 5 estrelas mais cobiçado do mundo.",
    iconName: "Palmtree",
    popular: true
  },
  {
    id: "paris-dinner",
    title: "Jantar Romântico na Torre Eiffel com Champagne",
    category: "Experiência Gastronómica",
    amount: "€ 350",
    description: "Uma noite inesquecível no restaurante Le Jules Verne, celebrando o início da vida a dois.",
    iconName: "Utensils",
    popular: false
  },
  {
    id: "wine-cellar",
    title: "Garrafa de Vinho do Porto Vintage do Ano de Núpcias",
    category: "Adega de Colecionador",
    amount: "€ 250",
    description: "Um tesouro enológico para o casal brindar nas bodas de estanho e prata.",
    iconName: "Wine",
    popular: false
  },
  {
    id: "spa-day",
    title: "Dia de Spa e Massagem Real para o Casal",
    category: "Bem-Estar & Relaxamento",
    amount: "€ 200",
    description: "Tratamento completo de ouro e cristais para relaxar após os preparativos do grande dia.",
    iconName: "Sparkles",
    popular: false
  },
  {
    id: "free-gift",
    title: "Presente em Dinheiro / Cota Personalizada",
    category: "Aporte Livre dos Noivos",
    amount: "Qualquer Valor",
    description: "Transferência direta via IBAN, Multicaixa Express ou PIX com mensagem dedicada ao casal.",
    iconName: "Gift",
    popular: true
  }
];

export const LOVE_STORY_MILESTONES = [
  {
    year: "O Primeiro Olhar",
    title: "Um Encontro de Destino",
    description: "Em uma noite iluminada, dois olhares se cruzaram e o mundo pareceu parar. Naquele instante, nasceu uma cumplicidade que desafia o tempo.",
    image: "https://images.unsplash.com/photo-1583939003579-730e3918a45a?w=800&q=80"
  },
  {
    year: "A Jornada",
    title: "Construindo um Império de Amor",
    description: "Viagens inesquecíveis, conquistas partilhadas, gargalhadas infinitas e a certeza diária de que a vida só faz sentido lado a lado.",
    image: "https://images.unsplash.com/photo-1515934751635-c81c6bc9a2d8?w=800&q=80"
  },
  {
    year: "O Grande 'Sim!'",
    title: "O Pedido Sob as Estrelas",
    description: "Em um cenário cinematográfico e com lágrimas de pura felicidade, Dionísio declarou seu amor eterno e Benedita disse o SIM que mudou seus destinos para sempre.",
    image: "https://images.unsplash.com/photo-1532712938310-34cb3982ef74?w=800&q=80"
  }
];
